...
def wirkungsorte():
    ...
    graph_count = (
        alt.Chart(df.nlargest(orte_filt, "Anzahl", keep="all"))
        .mark_bar()
        .encode(
            alt.X("Name:N", sort="y"),
            alt.Y("Anzahl"),
    ...
    INITIAL_VIEW_STATE = pdk.ViewState(
        latitude=50.67877877706058, longitude=8.129981238464392, zoom=4.5, max_zoom=16, bearing=0
    )
...
def wirkungsorte_musik():
    ...
    limiter = st.slider(
        "Jahresfilter", min_value=1400, max_value=int(musiker_orte["jahrzehnt"].max()), value=(1900), step=10
    )
...
def relationen():
    ...
    relation_count = (
        alt.Chart(rels.nlargest(rels_filt, "count", keep="all"))
        .mark_bar()
        .encode(
...
def systematik():
    ...
    classification_count = (
        alt.Chart(classification.nlargest(class_filt, "count", keep="all"))
        .mark_bar()
        .encode(
            ...
            tooltip=[
                alt.Tooltip("id", title="Notation"),
                alt.Tooltip("name", title="Bezeichnung"),
                alt.Tooltip("count", title="Anzahl"),
            ],
        )
    )
...
def systematik_ts():
    ...
    classification_ts_count = (
        alt.Chart(classification_ts.nlargest(class_ts_filt, "count", keep="all"))
        .mark_bar()
        .encode(
...
def zeitverlauf():
    ...
    created = (
        alt.Chart(created_at[f"{created_filt[0]}":f"{created_filt[1]}"].reset_index())
        .mark_line()
        .encode(
            alt.X("created_at:T", title="Erstelldatum"), alt.Y("count:Q", title="Sätze pro Monat"), tooltip=["count"]
        )
    )
    ...
    if satzart == "alle":
        ...
        entity_count = (
            ...
            .encode(
                ...
                tooltip=[
                    alt.Tooltip("entity", title="Satzart"),
                    alt.Tooltip("level", title="Katalogisierungslevel"),
                    alt.Tooltip("count", title="Anzahl"),
                ],
            )
        )
        ...
        entity_count = (
            alt.Chart(df.loc[df["entity"].str.startswith(satzart[:2])])
            .mark_bar()
            .encode(
                alt.X("sum(count)", title="Datensätze pro Katalogisierungslevel"),
                alt.Y("entity", title="Satzart"),
...
def newcomer():
    ...
    if satzart == "alle":
        ...
        newcomer = (
            alt.Chart(newcomer_daten.loc[newcomer_daten["bbg"].str.startswith(satzart[:2], na=False)])
            .mark_bar()
            .encode(
                alt.X("gnd_id:O", title="Entitäten", sort="-y"),
                alt.Y("count", title="Anzahl"),
...
def gnd_top():
    ...
    if satzart == "alle":
        ...
        gnd_top = (
            alt.Chart(top_daten.loc[top_daten["bbg"].str.startswith(satzart[:2], na=False)])
            .mark_bar()
            .encode(
                alt.X("gnd_id:N", title="Entitäten", sort="-y"),
                alt.Y("count:Q", title="Anzahl"),
...
def dnb_links():
    ...
    if satzart == "alle":
        ...
        st.write(
            f"{links.replace(',','.')} intellektuell vergebene Verknüpfungen zu {uniques_str.replace(',','.')} GND-Entitäten in den DNB-Titeldaten. Durchschnittlich {mean} GND-Verknüpfungen pro DNB-Titeldatensatz"
        )

        entity_df = pd.DataFrame.from_dict(
            {
                "intellektuell verknüpfte Entitäten": uniques,
                "Entitäten aus automatischen Prozessen": auto_entites,
                "Entitäten aus Fremddaten": fremd_entities,
            },
            orient="index",
        ).reset_index()
...
with st.container():
    st.info(
        "Hier finden Sie statistische Auswertungen der GND und ihrer Verknüpfungen mit den Titeldaten der Deutschen Nationalbibliothek (Stand der Daten: Januar 2022). Wählen Sie links die Satzart, die Sie interessiert, und Sie erhalten die verfügbaren Auswertungen und Statstiken. Verwenden Sie einen auf Chromium basierenden Browser."
    )
...
st.sidebar.info(
    "Diese Widgets haben die GitHub-User [niko2342](https://github.com/niko2342/), [ramonvoges](https://github.com/ramonvoges), [a-wendler](https://github.com/a-wendler/) sowie Christian Baumann geschrieben. Sie gehören zur Python Community der Deutschen Nationalbibliothek."
)

gnd_allgemein = st.container()
with gnd_allgemein:
